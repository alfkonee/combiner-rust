...
fn standardize_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    let (width, hieght) = get_smallest_dimension(image_1.dimensions(), image_2.dimensions());
    println!("Width: {}, Height: {}", width, hieght);

    if image_2.dimensions() == (width, hieght) {
        (image_1.resize_exact(width, hieght, Triangle), image_2)
    } else {
        (image_1, image_2.resize_exact(width, hieght, Triangle))
    }
}

fn get_smallest_dimension(dim_1: (u32, u32), dim_2: (u32, u32)) -> (u32, u32) {
    let pic_1 = dim_1.0 * dim_1.1;
    let pic_2 = dim_1.0 * dim_1.1;
